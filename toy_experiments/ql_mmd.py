...
class VAE(nn.Module):
    def __init__(
        self, state_dim, action_dim, latent_dim, max_action, device, hidden_dim=256
    ):
...
class QL_MMD(object):
    def __init__(
        ...
        self.vae = VAE(
            state_dim, action_dim, latent_dim, max_action, device, hidden_dim=hidden_dim
        ).to(device)
        self.vae_optimizer = torch.optim.Adam(self.vae.parameters(), lr=lr)

        self.actor = RegularActor(
            state_dim, action_dim, max_action, device, hidden_dim=hidden_dim
        ).to(device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=lr)

        if r_fun is None:
            self.critic = Critic(state_dim, action_dim, hidden_dim=hidden_dim).to(
                device
            )
        ...
        self.lagrange2_opt = torch.optim.Adam(
            [
                self.log_lagrange2,
            ],
            lr=1e-3,
    ...
    def train(self, replay_buffer, iterations, batch_size=100):
        for it in range(iterations):
            ...
            sampled_actions, raw_sampled_actions = self.vae.decode_multiple(
                state, num_decode=num_samples
            )  # B x N x d
            actor_actions, raw_actor_actions = self.actor.sample_multiple(
                state, num_sample=num_samples
            )  # num)

            mmd_loss = self.mmd_loss_laplacian(
                raw_sampled_actions.detach(), raw_actor_actions, sigma=self.mmd_sigma
            ).mean()
            ...
            thresh = 0.05
            lagrange_loss = (
                self.log_lagrange2.exp() * (mmd_loss.detach() - thresh)
            ).mean()
