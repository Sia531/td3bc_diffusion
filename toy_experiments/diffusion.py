...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, s, grad=True):
        if grad:
            ...
            x_recon = self.predict_start_from_noise(
                x, t=t, noise=self.model_frozen(x, t, s)
            )
    ...
    def guided_sample(
        self, state, q_fun, start=0.2, verbose=False, return_diffusion=False
    ):
        device = self.betas.device
        batch_size = state.shape[0]
        shape = (batch_size, self.action_dim)
        ...
        def guided_p_sample(x, t, s, fun):
            ...
            grads = torch.autograd.grad(
                outputs=q_value, inputs=a, create_graph=True, only_inputs=True
            )[0].detach()
            return (model_mean + model_log_variance * grads) + nonzero_mask * (
        ...
        if return_diffusion:
            return x, torch.stack(diffusion, dim=1).clamp_(
    ...
    def step_frozen(self):
        for param, target_param in zip(
            self.model.parameters(), self.model_frozen.parameters()
        ):
