...
std = 0.05
left_up_conor = (
    Normal(torch.tensor([-pos, pos]), torch.tensor([std, std]))
    .sample((200,))
    .clip(-1.0, 1.0)
    .numpy()
)
...
axs[0].scatter(
    left_bottom_conor[:, 0], left_bottom_conor[:, 1], label=r"$r \sim N (0.0, 0.5)$"
)
axs[0].scatter(
    right_up_conor[:, 0], right_up_conor[:, 1], label=r"$r \sim N (1.5, 0.5)$"
)
axs[0].scatter(
    right_bottom_conor[:, 0], right_bottom_conor[:, 1], label=r"$r \sim N (5.0, 0.5)$"
)
...
file_name = (
    f"ql_all_T{T}_eta{eta}_r_fun{args.r_fun}_lr{lr}_hd{hidden_dim}_mode_{args.mode}"
)
